...
import { selectDisplay, selectNextRequest, selectStatus, selectViewMode, selectIntersection, type SortOptions } from "../display/displaySlice";
...
export default function Pokemons() {
	...
	const tableInfoRef = useRef<TableInfoRef>({});
	const intersection = useAppSelector(selectIntersection, shallowEqual);

	const cachedDispaly = useMemo(() => {
		return display.map(id => Object.values(pokemons).find(pokemon => pokemon.id === id)) as PokemonData.Root[];
	}, [display, pokemons]);
